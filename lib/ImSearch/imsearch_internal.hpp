...
struct StrView {
    StrView() = default;
    StrView(const std::string &str) : mData(str.data()), mSize(static_cast<IndexT>(str.size())) {}
...
struct Output {
    ...
    static constexpr IndexT sDisplayEndFlag = static_cast<IndexT>(1)
...
float WeightedRatio(StrView s1, StrView s1Tokenised, StrView s2, StrView s2Tokenised, ReusableBuffers &buffers);
} // namespace ImSearch
