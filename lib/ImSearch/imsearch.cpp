...
static void AppendToDisplayOrder(const Input &input, ReusableBuffers &buffers, IndexT startInIndicesBuffer,
                                 IndexT endInIndicesBuffer, Output &output);
...
static bool DoDrawnGlyphsMatch(const ImDrawList &lhsList, const ImDrawIdx *lhsStart, const ImDrawList &rhsList,
                               const ImDrawIdx *rhsStart, int length);

static void HighlightSubstrings(const char *substrStart, const char *substrEnd, ImDrawList *drawList, int startIdxIdx,
                                int endIdxIdx);
...
ImSearch::ImSearchContext *ImSearch::GetCurrentContext() { return sContext; }

void ImSearch::SetCurrentContext(ImSearchContext *ctx) { sContext = ctx; }
...
void ImSearch::SearchBar(const char *hint) {
    ...
    ImGui::InputTextWithHint(
        "##SearchBar", hint, const_cast<char *>(userQuery.c_str()), userQuery.capacity() + 1,
        ...
        +[](ImGuiInputTextCallbackData *data) -> int {
            ...
            if (data->EventFlag == ImGuiInputTextFlags_CallbackCompletion) {
                ...
                data->DeleteChars(static_cast<int>(startOfLastToken),
                                  static_cast<int>(capturedQuery.size()) - static_cast<int>(startOfLastToken));
                data->InsertChars(static_cast<int>(startOfLastToken), newLastToken.c_str(),
                                  newLastToken.c_str() + newLastToken.size());
            }
            return 0;
        },
        &context);
    ...
    if (!userQuery.empty() && ImGui::IsItemFocused()) {
        ...
        if (!splitQuery.empty()) {
            StrView previewToDisplay =
                GetStringNeededToCompletePartial(splitQuery.back(), context.mResult.mOutput.mPreviewText);
            ...
            if (previewToDisplay.size() > 0) {
                ...
                drawList->AddText(completePreviewPos, disabledTextCol, previewToDisplay.begin(),
                                  previewToDisplay.end());
            }
        }
    }
}
...
bool ImSearch::Internal::PushSearchable(const char *name, void *functor, VTable vTable) {
    ...
    if (functor != nullptr && vTable != nullptr) {
...
void ImSearch::PopSearchable() { Internal::PopSearchable(nullptr, nullptr); }
...
void ImSearch::AddSynonym(const char *synonym) {
    ...
    IM_UNUSED(context);
    IM_ASSERT(context.mPushStackLevel > 0 &&
              "AddSynonym must be called between PushSearchable and PopSearchable. See imsearch_demo.cpp");
...
void ImSearch::PushStyleColor(ImSearchCol idx, ImU32 col) { PushStyleColor(idx, ImGui::ColorConvertU32ToFloat4(col)); }
...
void ImSearch::AssignInitialScores(const Input &input, ReusableBuffers &buffers) {
    ...
    for (IndexT i = 0; i < static_cast<IndexT>(input.mEntries.size()); i++) {
        ...
        float score =
            WeightedRatio(query, tokenSortedQuery, entry.mText, GetMemoizedTokenisedString(entry.mText), buffers);
...
void ImSearch::PropagateScoreToChildren(const Input &input, ReusableBuffers &buffers) {
    ...
    for (IndexT parentIndex = 0; parentIndex < static_cast<IndexT>(input.mEntries.size()); parentIndex++) {
        ...
        for (IndexT childIndex = parent.mIndexOfFirstChild; childIndex != sNullIndex;
...
void ImSearch::GenerateDisplayOrder(const Input &input, ReusableBuffers &buffers, Output &output) {
    ...
    for (IndexT i = 0; i < static_cast<IndexT>(input.mEntries.size()); i++) {
        if (input.mEntries[i].mIndexOfParent == sNullIndex && buffers.mScores[i] >= sCutOffStrength) {
            buffers.mTempIndices.emplace_back(i);
        }
    }

    AppendToDisplayOrder(input, buffers, 0, static_cast<IndexT>(buffers.mTempIndices.size()), output);
}

void ImSearch::AppendToDisplayOrder(const Input &input, ReusableBuffers &buffers, IndexT startInIndicesBuffer,
                                    IndexT endInIndicesBuffer, Output &output) {
    std::stable_sort(buffers.mTempIndices.begin() + startInIndicesBuffer,
                     buffers.mTempIndices.begin() + endInIndicesBuffer, [&](IndexT lhsIndex, IndexT rhsIndex) -> bool {
    ...
    for (IndexT indexInIndicesBuffer = startInIndicesBuffer; indexInIndicesBuffer < endInIndicesBuffer;
         indexInIndicesBuffer++) {
        ...
        for (IndexT childIndex = searchable.mIndexOfFirstChild; childIndex != sNullIndex;
             childIndex = input.mEntries[childIndex].mIndexOfNextSibling) {
        ...
        AppendToDisplayOrder(input, buffers, nextStartInIndicesBuffer, nextEndInIndicesBuffer, output);
...
bool ImSearch::DoDrawnGlyphsMatch(const ImDrawList &lhsList, const ImDrawIdx *lhsStart, const ImDrawList &rhsList,
                                  const ImDrawIdx *rhsStart, int length) {
...
ImSearch::Callback::Callback(void *originalFunctor, ImSearch::Internal::VTable vTable) : mVTable(vTable) {
    if (mVTable == nullptr) {
        return;
    }
...
ImSearch::Callback::Callback(Callback &&other) noexcept : mVTable(other.mVTable), mUserFunctor(other.mUserFunctor) {
...
ImSearch::Callback::~Callback() { ClearData(); }

bool ImSearch::Callback::operator()(const char *name) const {
    return InvokeAsPushSearchable(mVTable, mUserFunctor, name);
}

void ImSearch::Callback::operator()() const { InvokeAsPopSearchable(mVTable, mUserFunctor); }

bool ImSearch::Callback::InvokeAsPushSearchable(ImSearch::Internal::VTable vTable, void *userFunctor,
                                                const char *name) {
...
bool ImSearch::operator==(const StrView &lhs, const StrView &rhs) {
    return lhs.mSize == rhs.mSize &&
           ((lhs.mData == nullptr && rhs.mData == nullptr) || memcmp(lhs.mData, rhs.mData, lhs.mSize) == 0);
}

bool ImSearch::operator==(const Searchable &lhs, const Searchable &rhs) {
    return lhs.mText == rhs.mText && lhs.mIndexOfFirstChild == rhs.mIndexOfFirstChild &&
           lhs.mIndexOfLastChild == rhs.mIndexOfLastChild && lhs.mIndexOfParent == rhs.mIndexOfParent &&
           lhs.mIndexOfNextSibling == rhs.mIndexOfNextSibling;
}

bool ImSearch::operator==(const Input &lhs, const Input &rhs) {
    return lhs.mFlags == rhs.mFlags && lhs.mUserQuery == rhs.mUserQuery && lhs.mEntries == rhs.mEntries &&
...
ImSearch::LocalContext &ImSearch::GetLocalContext() {
    ...
    IM_ASSERT(!ImSearchContext.ContextStack.empty() &&
              "Not currently in between a ImSearch::BeginSearch and ImSearch::EndSearch");
...
bool ImSearch::CanCollectSubmissions() { return *ImSearch::GetUserQuery() != '\0'; }
...
const char *ImSearch::GetPreviewText() { return GetLocalContext().mResult.mOutput.mPreviewText.c_str(); }
...
void ImSearch::EndHighlightZone() {
    ...
    HighlightSubstrings(str->c_str(), str->c_str() + str->size(), drawList, prevIdxEnd, drawList->IdxBuffer.size());

    delete str;
}

void ImSearch::HighlightSubstrings(const char *substrStart, const char *substrEnd, ImDrawList *drawList,
                                   int startIdxIdx, int endIdxIdx) {
    ...
    struct DrawnGlyph {
        DrawnGlyph(ImDrawList &l, char character) {
            ...
            l.AddText({}, 0xffffffff, &character, &character + 1);
        ...
        for (int chIdx = 0; chIdx < substrEnd - substrStart; chIdx++) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (nextMatchEnd <= endIdxIdx &&
                    DoDrawnGlyphsMatch(queryDrawList, queryDrawList.IdxBuffer.Data + glyph.mIdxIdxStart, *drawList,
                                       &drawList->IdxBuffer[matchEnd], glyphLength)) {
...
ImSearch::StrView ImSearch::GetStringNeededToCompletePartial(StrView partial, StrView complete) {
    ...
    for (IndexT i = 0; i < partial.size(); i++) {
        if (std::tolower(static_cast<unsigned char>(tokenSubStr[i])) !=
            std::tolower(static_cast<unsigned char>(partial[i]))) {
            return {};
        }
    }
...
ImSearch::IndexT ImSearch::LevenshteinDistance(StrView s1, StrView s2, ReusableBuffers &buffers) {
    IndexT i;
    IndexT *row; /* we only need to keep one row of costs */
    IndexT *end;

    /* strip common prefix */
    while (s1.mSize > 0 && s2.mSize > 0 && *s1.mData == *s2.mData) {
        s1.mSize--;
        s2.mSize--;
        s1.mData++;
        s2.mData++;
...
float ImSearch::Ratio(StrView s1, StrView s2, ReusableBuffers &buffers) {
    const IndexT distance = LevenshteinDistance(s1, s2, buffers);
    const float ratio = 1.0f - static_cast<float>(distance) / static_cast<float>(s1.size() + s2.size());
    return ratio;
}

float ImSearch::PartialRatio(StrView shorter, StrView longer, ReusableBuffers &buffers) {
    ...
    for (IndexT i = 0; i <= longer.size() - shorter.size(); i++) {
        const float ratio = Ratio(shorter, {&longer[i], shorter.size()}, // Window into longer
                                  buffers);
...
float ImSearch::WeightedRatio(StrView s1, StrView s1Tokenised, StrView s2, StrView s2Tokenised,
    ...
    if (longerSize <= shorterSize + shorterSize / 2) {
        score = std::max(score, Ratio(s1Tokenised, s2Tokenised, buffers) * 0.95f);
    } else {
        const float weight = longerSize > shorterSize * 8 ? 0.5f : .8f;

        score = std::max(score, PartialRatio(s1, s2, buffers) * weight);

        score = std::max(score, PartialRatio(s1Tokenised, s2Tokenised, buffers) * 0.95f * weight);
    }
